...
def test_basic_flow_fan_out(
    csv_source, postgres_source, postgres_sink, csv_sink
):
    """
    Test a basic ETL pipeline
    """
    ...
    # 3. Load
    to_csv = ToCSV(path=csv_sink)
    to_sql = ToSQL(dsn_conn=postgres_sink)

    ###########
    # Pipeline
    FanIn(csv, postgres) >> merge >> fill_na >> FanOut(to_csv, to_sql)
